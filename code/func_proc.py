...
def main(argv=None):
    ...
    cmd='singularity run --cleanenv -B {templateflowdir}:$HOME/.cache/templateflow {sing} {work_dir_bids} {out_dir} participant --verbose -w {work_dir} --omp-nthreads {n_procs}  --fs-license-file {fs_license} --notrack --output-spaces MNI152NLin2009cAsym:res-2 --use-syn-sd'.format(templateflowdir = op.join(args.work_dir, 'templateflow'), sing=op.join(args.work_dir, fmriprep_file), work_dir_bids=op.join(args.work_dir, 'dset'), out_dir=op.join(args.work_dir, 'fmriprep-1.5.0'), work_dir=op.join(args.work_dir, 'fmriprep-work'), n_procs=args.n_procs, fs_license=op.join(args.work_dir, fs_license_file)) #c
    run(cmd)
