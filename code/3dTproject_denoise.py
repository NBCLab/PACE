...
def main(argv=None):
    ...
    if args.ses is not None:
        shutil.copytree(op.join(args.bids_dir, 'derivatives/dwidenoise-05.21.2019_fmriprep-1.5.0', args.sub, args.ses, 'fmriprep', args.sub), op.join(args.work_dir, args.sub))
    ...
    shutil.copyfile(op.join(op.dirname(op.dirname(args.bids_dir)), 'code', confound_script), op.join(args.work_dir, confound_script))
    shutil.copyfile(op.join(op.dirname(op.dirname(args.bids_dir)), 'code', censoring_script), op.join(args.work_dir, censoring_script))
    ...
    for tmp_rs_file in rest_files:
        ...
        nii_fn=glob(op.join(work_dir, 'func', '{0}*MNI152NLin2009cAsym*preproc_bold*nii.gz'.format(niifile)))[0]
        mask_fn=glob(op.join(work_dir, 'func', '{0}*MNI152NLin2009cAsym*brain_mask*nii.gz'.format(niifile)))[0]
    ...
    if rest_files:
        if args.ses is not None:
            shutil.copytree(op.join(args.work_dir, 'clean'), op.join(args.bids_dir, 'derivatives', '{0}'.format(args.deriv), args.sub, args.ses))
        else:
