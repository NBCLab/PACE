...
def nii2bids(bids_dir, raw_dir):
    ...
    work_dir : str
    """
    adult_dir = glob(op.join(raw_dir, "ENIGMA-PSA-Adult", "*", "*Adult"))
    kid_dir = glob(op.join(raw_dir, "ENIGMA-PSA-kid", "*", "*kid"))
    sourcedata = [adult_dir, kid_dir]
    modalities = {"anat": "T1w", "func": "task-rest_bold", "fmap": ["fieldmap", "magnitude"]}

    for data in sourcedata:
        for ses in data:
            raw_mod = "*structural"
            sub_raw_dirs = sorted(glob(op.join(ses, raw_mod, "*")))
            for sub_raw_dir in sub_raw_dirs:
                sub_id = sub_raw_dir.split("/")[-1].split("_")[0]
                for mod in modalities.keys():
                    ...
                    for in_file in in_files:
                        ...
                        if ext == ".gz":
                            ...
                            ext = ext2 + ext
                            if mod == "fmap":
                                fold = in_file.split("/")[-2]
                                if "fieldmaps" in fold:
                                    bids_name = f"{sub}_{modalities[mod][0]}{ext}"
                                elif "magnitudes" in fold:
